...
/*!
   Groups every frame's faces by color, interning colors across *all* frames so that a
   color appearing in only some frames still gets exactly one material.

   Splitting per color (rather than emitting one mesh with a UsdGeomSubset per material)
   keeps the writer simple, and matters more for animation: each material's mesh carries its
   own time-sampled topology, so a color that vanishes for a few frames simply has empty
   samples there.
 */
Scene buildScene(const std::vector<std::shared_ptr<const Geometry>>& frames, const Color4f& defaultColor)
{
  ...
  // Pass 1: intern colors across every frame, and record each face's material.
...
// by color rather than by body, so a multi-body model with different surface
                   ...
                   const UsdSurface& surface = {})
{
  ...
  // the same trip, which is why color goes through a bound material instead.
                ...
                std::ostream& output, const ExportInfo& exportInfo)
{
  ...
  // OpenSCAD is Z-up and its unit is the millimeter. USD defaults to Y-up and meters, so

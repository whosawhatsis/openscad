...
/*!
   Units per millimeter in the fine metric profile: 100 => 10um.

   16 bits buys a fixed product of range and resolution, and the millimeter
   ...
   run from tens to a few hundred millimeters - so a 15mm-deep object viewed from
   ...
   tooling assumes when it reads a depth PNG as millimeters. 1um was considered
...
/*!
   Units per millimeter for a profile, or 0 for the profiles that do not encode
   absolute distance.
...
//! of the distance to the model center. Deliberately close to 1: the cap breaks
...
/*!
   The depth range the shading normalizes across: a sphere centered on the
   **viewport's center of rotation** and large enough to contain the model, with
   ...
   **Orientation invariant by construction**, which is the point - a sphere
   ...
   Centered on the rotation center rather than the model's own center, because the
   camera orbits *that* point: the eye-to-center distance is then constant under
   rotation, and so is the radius the cap derives from it. Centered on the model
   ...
   enclose the bounding box from that center, so panning away from the model

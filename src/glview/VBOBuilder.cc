...
void VBOBuilder::create_surface(const PolySet& ps, const Transform3d& m, const Color4f& default_color,
                                ...
                                const Color4f *cutout_color, double smooth_angle)
{
  const std::shared_ptr<VertexData> vertex_data = data();
  ...
  for (size_t i = 0, n = ps.indices.size(); i < n; i++) {
    ...
    const int32_t color_index =
      has_colors && i < ps.color_indices.size() ? ps.color_indices[i] : PolySet::COLOR_INDEX_DEFAULT;
    const Color4f& color =
      !force_default_color && color_index >= 0 && static_cast<size_t>(color_index) < ps.colors.size() &&
          ps.colors[color_index].isValid()
        ? ps.colors[color_index]
        : (!force_default_color && color_index == PolySet::COLOR_INDEX_CUTOUT && cutout_color
             ? *cutout_color
             : default_color);
    material_finish_ = color_index >= 0 && static_cast<size_t>(color_index) < ps.finishes.size()
                          ? ps.finishes[color_index]
                          : default_finish;
    if (poly.size() == 3) {

// Surface bump on material() and color() (feature 15).
//
// Two distinct, independent attributes that are easy to confuse:
//
//   bump = scale                      procedural bump. Implicit 3D coherent
//   bump = [scale, strength]          noise sampled in object-local coords,
//   bump = [scale, strength, seed]    perturbing only the lighting normal.
...
material(c = "red", bump = 2) cube(10);
material(c = "red", bump = [2, 1, 0]) cube(10);

// 5. omitted seed is exactly equivalent to seed = 0
material(c = "green", bump = [3, 1]) cube(10);
material(c = "green", bump = [3, 1, 0]) cube(10);
...
// 9. roughness is a scalar and bump is a vector, so both coexist on one object
//    without ambiguity: visible bumps on a semi-glossy surface
material(c = "red", bump = [2, 0.5], roughness = 0.3, metallic = 0) cube(10);

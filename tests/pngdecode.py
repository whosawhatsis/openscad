...
def count_jumps(png, width, threshold=25):
    """Horizontally adjacent pixel pairs whose color jumps by more than
    `threshold` in any channel.

    A smoothly shaded surface produces these only at its silhouette. Transparent
    geometry drawn without depth ordering produces them all over its interior,
    because each triangle writes depth and occludes the ones behind it, so the
    interior fills with stripes. That difference is large - measured 9413
    against 652 for the same scene - so this is a blunt instrument used well
    inside its resolution."""
    px = pixels(png)
    n = 0
    for y in range(len(px) // width):
